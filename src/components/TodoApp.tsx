...
type TaskDraft = { title:string; description:string; priority:Priority; dueDate:string; dueTime:string; reminderMinutes:string; projectId:string; tags:string };
const emptyDraft: TaskDraft = { title:"", description:"", priority:"medium", dueDate:"", dueTime:"", reminderMinutes:"", projectId:"", tags:"" };
...
function TaskModal({task,projects,onClose,onSave}:{task?:Task;projects:Project[];onClose:()=>void;onSave:(d:TaskDraft)=>Promise<boolean>}) {
  const [draft,setDraft]=useState<TaskDraft>(task ? {title:task.title,description:task.description||"",priority:task.priority,dueDate:task.dueDate||"",dueTime:task.dueTime?.slice(0,5)||"",reminderMinutes:task.reminderMinutes?.toString()||"",projectId:task.projectId||"",tags:(task.tags||[]).join(", ")} : emptyDraft);
...
export function TodoApp({ user }: { user: AuthUser }) {
  ...
  const save=async(d:TaskDraft)=>{const dueAt=d.dueDate&&d.dueTime?new Date(`${d.dueDate}T${d.dueTime}`).toISOString():null;const input={title:d.title,description:d.description,priority:d.priority,dueDate:d.dueDate||null,dueTime:d.dueDate&&d.dueTime?d.dueTime:null,dueAt,reminderMinutes:d.reminderMinutes?Number(d.reminderMinutes) as 15|60|1440:null,projectId:d.projectId||null,tags:d.tags.split(",").map(x=>x.trim()).filter(Boolean)};try{if(modal==="edit"&&selected){await updateTask(selected.id,input);notify("Task updated")}else{await createTask(input);notify("Task created")}setModal(null);setSelected(undefined);return true}catch(error){reportMutationError("save the task",error);return false}};
