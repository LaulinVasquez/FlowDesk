...
function TaskModal({task,projects,connections,userId,onClose,onSave}:{task?:Task;projects:Project[];connections:Connection[];userId:string;onClose:()=>void;onSave:(d:TaskDraft)=>Promise<boolean>}) {
  const [draft,setDraft]=useState<TaskDraft>(task ? {title:task.title,description:task.description||"",priority:task.priority,dueDate:task.dueDate||"",dueTime:task.dueTime?.slice(0,5)||"",reminderMinutes:task.reminderMinutes?.toString()||"",projectId:task.projectId||"",assignedUserId:task.assignedUserId||"",tags:(task.tags||[]).join(", ")} : emptyDraft);
  const people=connections.filter(connection=>connection.status==="accepted").map(connection=>connection.requesterId===userId?connection.addressee:connection.requester);
    ...
    <section className="modal task-modal" role="dialog" aria-modal="true" aria-labelledby="task-title">
      ...
      <form onSubmit={submit}>
        ...
        <div className="form-grid">
          ...
          <label>Assign to<select value={draft.assignedUserId} onChange={e=>setDraft({...draft,assignedUserId:e.target.value})}><option value="">Unassigned</option>{people.map(person=><option key={person.id} value={person.id}>{person.fullName}</option>)}</select></label>
...
export function TodoApp({ user }: { user: AuthUser }) {
  ...
  useEffect(()=>{const taskId=new URLSearchParams(window.location.search).get("task");if(!taskId)return;const task=tasks.find(item=>item.id===taskId);if(task){setSelected(task);setModal("edit")}},[tasks]);
  useEffect(()=>{
  ...
  const save=async(d:TaskDraft)=>{const dueAt=d.dueDate&&d.dueTime?new Date(`${d.dueDate}T${d.dueTime}`).toISOString():null;const input={title:d.title,description:d.description,priority:d.priority,dueDate:d.dueDate||null,dueTime:d.dueDate&&d.dueTime?d.dueTime:null,dueAt,reminderMinutes:d.reminderMinutes?Number(d.reminderMinutes) as 15|60|1440:null,projectId:d.projectId||null,assignedUserId:d.assignedUserId||null,tags:d.tags.split(",").map(x=>x.trim()).filter(Boolean)};try{if(modal==="edit"&&selected){await updateTask(selected.id,input);notify("Task updated")}else{await createTask(input);notify("Task created")}setModal(null);setSelected(undefined);return true}catch(error){reportMutationError("save the task",error);return false}};
  ...
  const changeStage=async(task:Task,stage:TaskStage)=>{if(pendingTasks.has(task.id))return;setPendingTasks(current=>new Set(current).add(task.id));try{await updateTaskStage(task.id,stage);notify(`Task moved to ${stage}`)}catch(error){reportMutationError("update the task stage",error)}finally{setPendingTasks(current=>{const next=new Set(current);next.delete(task.id);return next})}};
  ...
  return <div className={`app-shell ${collapsed?"is-collapsed":""}`}>
        ...
        <>
          ...
          <section className="task-section">
            ...
            {filtered.length?<div className="task-list"><div className="table-head"><span>Task</span><span>Project</span><span>Priority</span><span>Due date</span><span/></div>{filtered.map(task=><div className={`task-row ${task.completed?"done":""}`} key={task.id}>
              ...
              <div className="row-actions">{task.assignedUserId===user.id&&task.stage==="assigned"&&<button className="stage-action" onClick={()=>changeStage(task,"working")}>Start work</button>}{task.assignedUserId===user.id&&task.stage==="working"&&<button className="stage-action" onClick={()=>changeStage(task,"reviewed")}>Submit review</button>}{task.ownerId===user.id&&task.stage==="reviewed"&&<button className="stage-action" onClick={()=>changeStage(task,"approved")}>Approve</button>}{task.ownerId===user.id&&<button aria-label="Edit task" onClick={()=>{setSelected(task);setModal("edit")}}><Pencil/></button>}{task.ownerId===user.id&&<button aria-label="Delete task" onClick={()=>setDeleting(task)}><Trash2/></button>}<button aria-label="More actions"><MoreHorizontal/></button></div>
    ...
    {modal&&<TaskModal task={modal==="edit"?selected:undefined} projects={projects} connections={connections} userId={user.id} onClose={()=>{setModal(null);setSelected(undefined)}} onSave={save}/>}
...
function PeopleView({connections,userId,request,answer,remove,notify}:{connections:Connection[];userId:string;request:(email:string)=>Promise<void>;answer:(id:string,response:"accepted"|"rejected")=>Promise<void>;remove:(id:string)=>Promise<void>;notify:(message:string)=>void}) {
  const [email,setEmail]=useState(""),[pending,setPending]=useState(false),[error,setError]=useState("");
  const run=async(action:()=>Promise<void>,success:string)=>{setPending(true);setError("");try{await action();notify(success)}catch(value){setError(value instanceof Error?value.message:"Unable to update connection.")}finally{setPending(false)}};
  const submit=async(event:FormEvent)=>{event.preventDefault();if(!email.trim())return;await run(()=>request(email),"Connection request sent");setEmail("")};
  const incoming=connections.filter(connection=>connection.status==="pending"&&connection.addresseeId===userId);
  const others=connections.filter(connection=>!incoming.includes(connection));
  const person=(connection:Connection)=>connection.requesterId===userId?connection.addressee:connection.requester;
  return <section className="people-view"><div className="manage-title"><div><h2>Your network</h2><p>Connect with teammates and assign work securely.</p></div></div><form className="connect-form" onSubmit={submit}><UserPlus/><input type="email" value={email} onChange={event=>setEmail(event.target.value)} placeholder="teammate@example.com" aria-label="User email"/><button className="btn primary" disabled={pending||!email.trim()}>{pending?<Loader2 className="spin"/>:<Plus/>}Connect</button></form>{error&&<p className="connection-error" role="alert">{error}</p>}
    {incoming.length>0&&<><h3 className="people-heading">CONNECTION REQUESTS</h3><div className="people-grid">{incoming.map(connection=><article className="person-card" key={connection.id}><div className="person-avatar">{person(connection).fullName.slice(0,2).toUpperCase()}</div><div><strong>{person(connection).fullName}</strong><span>{person(connection).email}</span></div><div className="person-actions"><button className="btn primary" disabled={pending} onClick={()=>run(()=>answer(connection.id,"accepted"),"Connection accepted")}>Accept</button><button className="btn secondary" disabled={pending} onClick={()=>run(()=>answer(connection.id,"rejected"),"Request declined")}>Decline</button></div></article>)}</div></>}
    <h3 className="people-heading">CONNECTIONS</h3>{others.length?<div className="people-grid">{others.map(connection=><article className="person-card" key={connection.id}><div className="person-avatar">{person(connection).fullName.slice(0,2).toUpperCase()}</div><div><strong>{person(connection).fullName}</strong><span>{person(connection).email}</span><small>{connection.status==="accepted"?"Connected":"Request pending"}</small></div><button className="icon-btn" disabled={pending} aria-label={`Remove ${person(connection).fullName}`} onClick={()=>run(()=>remove(connection.id),"Connection removed")}><X/></button></article>)}</div>:<div className="people-empty"><Users/><h3>No connections yet</h3><p>Connect by exact email to begin assigning tasks.</p></div>}
  </section>;
}
